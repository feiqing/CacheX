...
public class InnerMapService {
    ...
    public Map<Integer, User> unmodifiableMap(@CacheKey(value = "id:") List<Integer> ids) {
        if (ids.size() == 1) {
            int id = ids.get(0);
    ...
    @Cached(expire = Expire.TEN_MIN)
    public Map<Integer, User> synchronizedMap(@CacheKey(value = "id:") List<Integer> ids) {
        Map<Integer, User> map = new HashMap<>();
        for (Integer id : ids) {
            map.put(id, new User(id, "name" + id, new Date(), id, ""));
        }
    ...
    @Cached(expire = Expire.TEN_MIN)
    public Map<Integer, User> checkedMap(@CacheKey(value = "id:") List<Integer> ids) {
    ...
    public Map<Integer, User> immutableMap(@CacheKey(value = "id:") List<Integer> ids) {
        TreeMap<Integer, User> map = new TreeMap<>();
        for (Integer id : ids) {
            map.put(id, new User(id, "name" + id, new Date(), id, ""));
        }
